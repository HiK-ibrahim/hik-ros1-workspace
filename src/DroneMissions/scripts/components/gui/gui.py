...
class FusionGUI(QWidget):
    ...
    def update_views(self):
        # Lidar ve ön kamera füzyonunu güncelle
        if self.last_lidar_msg and self.last_front_camera_msg:
            try:
                # Lidar maskesi
                lidar_mask = process_fusion(self.last_lidar_msg, None)  # Sadece lidar verisi

                # Ön kamera görüntüsü
                front_camera = self.bridge.imgmsg_to_cv2(self.last_front_camera_msg, "bgr8")

                # Füzyon: Maskeyi ön kamera görüntüsüne ekle
                ...
                rospy.logwarn(f"Füzyon hatası: {e}")

        # Downward kameradan çizgi tespitli görüntüyü güncelle
        if self.last_downward_camera_msg:
            try:
                ...
                rospy.logwarn(f"Downward kamera işlem hatası: {e}")
