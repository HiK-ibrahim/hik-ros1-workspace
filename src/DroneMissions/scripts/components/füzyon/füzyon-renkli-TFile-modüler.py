...
def get_transform_matrix(target_frame, source_frame):
    try:
        ...
        trans_matrix[0:3, 3] = [translation.x, translation.y, translation.z]
        ...
        rospy.logwarn(f"TF dönüşüm hatası: {ex}")
        return None


def create_lidar_layer(scan):
    lidar_layer = np.zeros((res_y, res_x, 3), dtype=np.uint8)  # Siyah arka plan
    transform_matrix = get_transform_matrix("front_cam_optical_frame", "laser0_frame")
